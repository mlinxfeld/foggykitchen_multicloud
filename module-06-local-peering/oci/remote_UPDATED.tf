 ...
 resource "null_resource" "foggykitchen_access_db_from_backend" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo sed -i 's/fkadbpe\\.adb\\.${var.region}\\.oraclecloud\\.com/${module.oci-foggykitchen-adb.adb_database.private_endpoint_ip}/g' /usr/lib/oracle/21/client64/lib/network/admin/tnsnames.ora",
