...
resource "oci_core_security_list" "foggykitchen_backend_seclist" {
  ...
  ingress_security_rules {
    protocol = "6" # TCP
    source   = "10.1.0.0/24" # Allow SSH traffic from VCN3 (region2)
...
resource "oci_core_network_security_group_security_rule" "foggykitchen_adb_nsg_egress_group_sec_rule2" {
  network_security_group_id = oci_core_network_security_group.foggykitchen_adb_nsg.id
  direction                 = "EGRESS"
  protocol                  = "6"
...
resource "oci_core_network_security_group_security_rule" "fk_adb_nsg_ingress_group_sec_rule" {
  ...
  direction                 = "INGRESS"
  protocol                  = "6"
  source                    = "10.0.2.0/24" # access from backend_vms 
  source_type               = "CIDR_BLOCK"
  tcp_options {
    destination_port_range {
      max = 1522
...
resource "oci_core_network_security_group_security_rule" "fk_adb_nsg_ingress_group_sec_rule2" {
  network_security_group_id = oci_core_network_security_group.foggykitchen_adb_nsg.id
  direction                 = "INGRESS"
  protocol                  = "6"
  source                    = "10.2.1.0/24" # access from analytical_vm 
  source_type               = "CIDR_BLOCK"
  tcp_options {
    destination_port_range {
      max = 1522
