...
resource "null_resource" "foggykitchen_provision_backend" {
 ...
 resource "null_resource" "foggykitchen_access_db_from_analytics_vm" {

  depends_on = [module.oci-foggykitchen-adb] #null_resource.foggykitchen_access_db_from_backend]

  connection {
    type                = "ssh"
    user                = "opc"
    private_key         = tls_private_key.public_private_key_pair.private_key_pem
    host                = oci_core_instance.foggykitchen_analytical_vm.private_ip
    bastion_host        = oci_core_instance.foggykitchen_bastion_vm2.public_ip
    bastion_user        = "opc"
    bastion_private_key = tls_private_key.public_private_key_pair.private_key_pem
  }
