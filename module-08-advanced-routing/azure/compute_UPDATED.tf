...
resource "azurerm_linux_virtual_machine" "foggykitchen_analytical_vm" {
  name                = "foggykitchen_analytical_vm"
  computer_name       = "fkanalyticalvm"
  location            = azurerm_resource_group.foggykitchen_rg2.location
  resource_group_name = azurerm_resource_group.foggykitchen_rg2.name
  size                = var.vm_size
  admin_username      = "azureuser"
  network_interface_ids = [
    azurerm_network_interface.foggykitchen_analytical_nic.id
  ]
